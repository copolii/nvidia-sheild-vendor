...
ifeq ($(LOCAL_NVIDIA_NVMAKE_OVERRIDE_TOP),)
  NVIDIA_NVMAKE_TOP := $(TEGRA_TOP)/gpu/drv
else
  NVIDIA_NVMAKE_TOP := $(LOCAL_NVIDIA_NVMAKE_OVERRIDE_TOP)
endif

NVIDIA_NVMAKE_MODULE := $(NVIDIA_NVMAKE_TOP)/drivers/$(LOCAL_NVIDIA_NVMAKE_BUILD_DIR)/_out/Android_ARMv7_$(TARGET_BUILD_TYPE)/$(LOCAL_MODULE)$(LOCAL_MODULE_SUFFIX)
...
$(NVIDIA_NVMAKE_MODULE) $(LOCAL_MODULE)_nvmakeclean: NVIDIA_NVMAKE_COMMAND := $(MAKE) \
    ...
    NV_BUILD_TYPE=$(TARGET_BUILD_TYPE) \
    $(NVIDIA_NVMAKE_VERBOSE) \
    -C $(NVIDIA_NVMAKE_TOP)/drivers/$(LOCAL_NVIDIA_NVMAKE_BUILD_DIR) \
    -f makefile.nvmk \
